...
half4 SSAO(Varyings input) : SV_Target
{
    ...
    for (int s = 0; s < SAMPLE_COUNT; s++)
    {
        ...
        half2 uv_s1_01;
            ...
            uv_s1_01 = (spos_s1 + HALF_ONE) * HALF_HALF;
#else
        zDist = half(-dot(UNITY_MATRIX_V[2].xyz, vpos_s1));
        uv_s1_01 = half2(spos_s1 * rcp(zDist) + HALF_ONE) * HALF_HALF;
#endif

        // Check if sample goes outside screen BEFORE clamping
        bool insideScreen = (uv_s1_01.x >= 0.0 && uv_s1_01.x <= 1.0 && uv_s1_01.y >= 0.0 && uv_s1_01.y <= 1.0);

        // Clamp coords for safe sampling (avoids artifacts on borders)
        const float pad = 0.001;
        float2 uv_s1_safe = clamp(uv_s1_01, pad, 1.0 - pad);

        // Relative depth of the sample point
        float rawDepth_s = SampleDepth(uv_s1_safe);
        ...
        // Если сэмпл за пределами экрана — обнуляем его вклад
        if (!insideScreen)
            isInsideRadius = 0.0;

        // Relative position of the sample point
        half3 v_s2 = half3(ReconstructViewPos(uv_s1_safe, linearDepth_s) - vpos_o);
    ...
    // Optional: light fade near extreme edges (чтобы смягчить переход, а не рамку)
    const float edgeFade = 0.001; // ~1% экрана
    float fx = smoothstep(0.0, edgeFade, uv.x) * smoothstep(1.0, 1.0 - edgeFade, uv.x);
    float fy = smoothstep(0.0, edgeFade, uv.y) * smoothstep(1.0, 1.0 - edgeFade, uv.y);
    ao *= (fx * fy);
